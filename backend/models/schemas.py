...
class StreamStartRequest(BaseModel):
    ...
    max_turns: Optional[int] = Field(
        default=5, ge=1, le=100, description="Number of turns to run"
    )
...
class AgentConfig(BaseModel):
    ...
    avatar_talk_small: Optional[str] = Field(
        None, description="Path to small mouth avatar"
    )
    avatar_talk_medium: Optional[str] = Field(
        None, description="Path to medium mouth avatar"
    )
...
class ConfigUpdate(BaseModel):
    ...
    max_turns: Optional[int] = Field(
        None, ge=1, le=100, description="Default number of turns"
    )
    pause_between_turns: Optional[float] = Field(
        None, ge=0, le=10, description="Pause duration in seconds"
    )
...
class TranscriptMessage(BaseModel):
    ...
    topic: Optional[str] = Field(
        None, description="Discussion topic (if this is a topic change)"
    )
